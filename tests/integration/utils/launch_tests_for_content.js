  ...
  describe("API tests", () => {
    ...
    describe("loadVideo", () => {
      it("should fetch the manifest then the init segments", async function () {
        ...
        const firstPeriodAdaptationsInfos =
          periodsInfos[firstPeriodIndex].adaptations;
        ...
        if (
          ...
          if (
            ...
            expect(xhrMock.getLockedXHR().length).to.be.at.least(
              2,
              "should request two init segments"
            );
            ...
            expect(xhrMock.getLockedXHR().length).to.equal(1);
            expect(xhrMock.getLockedXHR()[0].url).to.equal(
              videoRepresentationInfos.index.init.url
            );
          } else {
            expect(xhrMock.getLockedXHR().length).to.equal(1);
    ...
    describe("getWallClockTime", () => {
      ...
      it("should return the starting position if one", async () => {
        ...
        await waitForLoadedStateAfterLoadVideo(player);
        expect(player.getWallClockTime()).to.be.closeTo(
          minimumPosition + 4,
          0.001
        );
      });
    ...
    describe("setPlaybackRate", () => {
      ...
      it("should update the speed accordingly", async function () {
        ...
        expect(secondPosition).to.be.closeTo(
          initialPosition + duration2 * 3,
          2
        );
      });
    });
            ...
            for (let j = 0; j < audioTracks.length; j++) {
              ...
              if (audioTrack.id === adaptation.id) {
                ...
                expect(audioTrack.active).to.equal(
                  activeAudioTrack
                    ? activeAudioTrack.id === audioTrack.id
                    : false
                );
              }
            }
            ...
            for (let j = 0; j < videoTracks.length; j++) {
              ...
              if (videoTrack.id === adaptation.id) {
                ...
                for (
                  ...
                  expect(reprTrack.frameRate).to.equal(
                ...
                expect(videoTrack.active).to.equal(
                  activeVideoTrack
                    ? activeVideoTrack.id === videoTrack.id
                    : false
                );
              }
            }
    ...
    describe("setWantedBufferAhead", () => {
      ...
      it("should download until a set wanted buffer ahead", async function () {
        ...
        expect(buffered.start(1)).to.be.at.most(
          minimumPosition + 10 + 20 + 10 + 10
        );
