...
const audioSegments = flatMap(
  ...
  (time) => {
    return [
      ...
      {
        url:
          BASE_URL + `QualityLevels96000/Fragmentsaudio_und=${time + 90000000}`,
        path: path.join(__dirname, `./media/Fragments(audio_und=${time}).mp4`),
        contentType: "video/mp4",
      },
    ];
        ...
        return [
          {
            ...
            path: path.join(
              __dirname,
              `./media/${quality}-Fragments(video=0).mp4`
            ),
            contentType: "video/mp4",
          },
          {
            url:
              BASE_URL +
              `QualityLevels${quality}/Fragmentsvideo=${time + 90000000}`,
            path: path.join(
              __dirname,
              `./media/${quality}-Fragments(video=0).mp4`
            ),
          ...
          },
          {
            url:
              BASE_URL +
              `QualityLevels${quality},hardwareProfile=${hwProfile}/Fragmentsvideo=${time}`,
            path: path.join(
              __dirname,
              `./media/${quality}-Fragments(video=0).mp4`
            ),
    ...
    segments.push(
      {
        ...
        path: path.join(
          __dirname,
          `./media/${quality}-Fragments(video=740000000).mp4`
        ),
        contentType: "video/mp4",
      },
      {
        url:
          BASE_URL +
          `QualityLevels${quality},hardwareProfile=${hwProfile}/Fragmentsvideo=740000000`,
        path: path.join(
          __dirname,
          `./media/${quality}-Fragments(video=740000000).mp4`
        ),
