...
Object.assign(FakeXMLHttpRequest.prototype, Events.EventTarget, {
  ...
  downloadProgress(progressEventRaw) {
    this.dispatchEvent(
      new Events.ProgressEvent("progress", progressEventRaw, this)
    );
  },

  uploadError(error) {
    this.upload.dispatchEvent(
      new Events.CustomEvent("error", { detail: error })
    );
  },
...
function updateReadyState(fakeXhr, state) {
  ...
  if (fakeXhr.readyState === FakeXMLHttpRequest.DONE) {
    ...
    fakeXhr.upload.dispatchEvent(
      new Events.ProgressEvent("progress", progress, fakeXhr)
    );
    fakeXhr.upload.dispatchEvent(
      new Events.ProgressEvent(evtName, progress, fakeXhr)
    );
    fakeXhr.upload.dispatchEvent(
      new Events.ProgressEvent("loadend", progress, fakeXhr)
    );

    fakeXhr.dispatchEvent(
      new Events.ProgressEvent("progress", progress, fakeXhr)
    );
...
function setResponseBody(fakeXhr, body) {
  ...
  const isTextResponse =
    fakeXhr.responseType === "" || fakeXhr.responseType === "text";
  ...
  fakeXhr.response = convertResponseBody(
...
function isXmlContentType(contentType) {
  return (
    !contentType || /(text\/xml)|(application\/xml)|(\+xml)/.test(contentType)
  );
}
