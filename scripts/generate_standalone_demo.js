...
console.log(
  `\x1b[35m[${getHumanReadableHours()}]\x1b[0m ` + "Building demo..."
);
...
const compilerWatching = compiler.watch(
  ...
  (err, stats) => {
    ...
    if (
      ...
      /* eslint-disable no-console */
      console.log(
        `\x1b[32m[${getHumanReadableHours()}]\x1b[0m Demo built (in ${
          stats.endTime - stats.startTime
        } ms).`
      );
      /* eslint-enable no-console */
...
compilerWatching.compiler.hooks.watchRun.intercept({
  call() {
    /* eslint-disable no-console */
    console.log(
      `\x1b[35m[${getHumanReadableHours()}]\x1b[0m ` + "Re-building demo"
    );
    /* eslint-enable no-console */
  },
});
