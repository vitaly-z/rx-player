...
const linkPlayerEventsToState = (player, state, $destroy) => {
  ...
  intervalObservable(POSITION_UPDATES_INTERVAL)
    .pipe(
      map(() => {
  ...
  fromPlayerEvent(player, "brokenRepresentationsLock")
    .pipe(takeUntil($destroy))
    .subscribe((evt) => {
  ...
  fromPlayerEvent(player, "videoTrackChange")
    .pipe(takeUntil($destroy))
  ...
  fromPlayerEvent(player, "audioTrackChange")
    .pipe(takeUntil($destroy))
    .subscribe(() => {
  ...
  fromPlayerEvent(player, "playerStateChange")
  ...
  fromPlayerEvent(player, "warning")
    .pipe(takeUntil($destroy))
    .subscribe((warning) => {
