  ...
  describe("guidToUuid", () => {
    ...
    it("should translate PlayReady GUID to universal UUID", () => {
      ...
      expect(strUtils.guidToUuid(uuid1)).toEqual(
        new Uint8Array([
          76, 175, 27, 15, 184, 7, 73, 156, 181, 133, 213, 230, 192, 48, 134,
          31,
        ])
      );
  ...
  describe("utf8ToStr", () => {
    ...
    it("should translate sequence of UTF-8 code units into the corresponding string", () => {
      ...
      expect(
        strUtils.utf8ToStr(
          new Uint8Array([
            0xf0, 0x9f, 0x98, 0x80, 0xf0, 0x9f, 0x90, 0x80, 0xe1, 0xbc, 0x80,
            0x65,
          ])
        )
      ).toBe("😀🐀ἀe");
    ...
    it("should strip off the UTF8 BOM if present", () => {
      expect(
        strUtils.utf8ToStr(
          new Uint8Array([
            0xef, 0xbb, 0xbf, 0xf0, 0x9f, 0x98, 0x80, 0xf0, 0x9f, 0x90, 0x80,
            0xe1, 0xbc, 0x80, 0x65,
          ])
        )
      ).toBe("😀🐀ἀe");
