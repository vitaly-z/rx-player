...
describe("utils - ranges", () => {
  ...
  describe("keepRangeIntersection", () => {
    ...
    it("should return the other range if one of it contains the other", () => {
      ...
      expect(keepRangeIntersection(timeRanges1, timeRanges2)).toEqual(
        timeRanges1
      );
      expect(keepRangeIntersection(timeRanges2, timeRanges1)).toEqual(
        timeRanges1
      );
    });
  ...
  describe("isAfter", () => {
    ...
    it("should return true if the first range begins at the same time than the end of the second range", () => {
      ...
      expect(isAfter({ start: 70, end: 70 }, { start: 10, end: 70 })).toBe(
        true
      );
    });
    ...
    it("should return false if the first range begins before the end of the second range", () => {
      /* eslint-enable max-len */
      expect(isAfter({ start: 10.1, end: 10.2 }, { start: 10, end: 70 })).toBe(
        false
      );
      expect(isAfter({ start: 19, end: 50 }, { start: 10, end: 20 })).toBe(
        false
      );
  ...
  describe("isBefore", () => {
    ...
    it("should return true if the first range ends at the same time than the start of the second range", () => {
      ...
      expect(isBefore({ start: 10, end: 70 }, { start: 70, end: 70 })).toBe(
        true
      );
    });
    ...
    it("should return false if the first range ends after the start of the second range", () => {
      /* eslint-enable max-len */
      expect(isBefore({ start: 10, end: 70 }, { start: 10.1, end: 10.2 })).toBe(
        false
      );
      expect(isBefore({ start: 10, end: 20 }, { start: 19, end: 50 })).toBe(
        false
      );
  ...
  describe("insertInto", () => {
    it("should do nothing if the given range is empty", () => {
      ...
      expect(
        insertInto([{ start: 0, end: 0 }], { start: 10, end: 10 })
      ).toEqual([{ start: 0, end: 0 }]);
      expect(
        insertInto([{ start: 0, end: 9 }], { start: 10, end: 10 })
      ).toEqual([{ start: 0, end: 9 }]);
    });

    it("should just add the range if we had no original ranges", () => {
      expect(insertInto([], { start: 0, end: 5 })).toEqual([
        { start: 0, end: 5 },
      ]);
    });
    ...
    it("should not add a range contained entirely in a previous one", () => {
      expect(
        insertInto([{ start: 0, end: 100 }], { start: 10, end: 20 })
    ...
    it("should merge a range contained partially in a previous one", () => {
      expect(
        insertInto([{ start: 0, end: 100 }], { start: 10, end: 110 })
    ...
    it("should merge a range contiguous with a previous one", () => {
      expect(
        insertInto([{ start: 0, end: 100 }], { start: 100, end: 110 })
      ).toEqual([{ start: 0, end: 110 }]);
      expect(
        insertInto([{ start: 50, end: 70 }], { start: 0, end: 50 })
      ).toEqual([{ start: 0, end: 70 }]);
      expect(
        insertInto(
          [
            { start: 0, end: 100 },
