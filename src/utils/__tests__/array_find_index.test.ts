...
const initialArrayFindIndex = (Array.prototype as { findIndex: unknown })
  .findIndex;
...
describe("utils - arrayFindIndex", () => {
  ...
  afterEach(() => {
    (Array.prototype as { findIndex: unknown }).findIndex =
      initialArrayFindIndex;
  });
  ...
  if (typeof initialArrayFindIndex === "function") {
    it("should call the original array.findIndex function if it exists", () => {
      ...
      const spy = jest.spyOn(
        arr as unknown as { findIndex: () => unknown },
        "findIndex"
      );
