...
export default function fetchRequest(
  ...
  const abortController: AbortController | null = !isNullOrUndefined(
    _AbortController
  )
  ...
  const deregisterCancelLstnr = options.cancelSignal.register(
  ...
  fetchOpts.signal = !isNullOrUndefined(abortController)
