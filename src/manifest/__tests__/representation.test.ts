...
describe("Manifest - Representation", () => {
  ...
  it("should be able to return an exploitable codecs + mimeType string", () => {
    ...
    expect(
      new Representation(args1, { type: "audio" }).getMimeTypeString()
    ).toBe(';codecs=""');
    ...
    expect(
      new Representation(args2, { type: "audio" }).getMimeTypeString()
    ).toBe('foo;codecs=""');
    ...
    expect(
      new Representation(args3, { type: "audio" }).getMimeTypeString()
    ).toBe(';codecs="bar"');
    ...
    expect(
      new Representation(args4, { type: "audio" }).getMimeTypeString()
    ).toBe('foo;codecs="bar"');
  });
  ...
  it("should set `isSupported` of non-supported codecs or mime-type to `false`", () => {
    ...
    expect(mockIsCodecSupportedFalse).toHaveBeenCalledWith(
      'audio/mp4;codecs="mp4a.40.2"'
    );
  });
