...
import probeMediaConfiguration, {
  IBrowserAPIS,
} from "./probeMediaConfiguration";
...
function getStatusFromConfiguration(
  ...
  return probeMediaConfiguration(config, browserAPIS).then(
    ({ globalStatus }) => {
      switch (globalStatus) {
  ...
  getStatusForHDCP(hdcp: string): Promise<string> {
    if (hdcp === undefined || hdcp.length === 0) {
      return Promise.reject(
        "MediaCapabilitiesProbers >>> Bad Arguments: " +
