...
function createAVC1Box(
  ...
  return createBox(
    ...
    concat(
      6, // 6 bytes reserved
      itobe2(1),
      16, // drefIdx + QuickTime reserved, zeroes
      itobe2(width), // size 2 w
      itobe2(height), // size 2 h
      itobe2(hRes),
      2, // reso 4 h
      itobe2(vRes),
      2 + 4, // reso 4 v + QuickTime reserved, zeroes
...
function createSCHMBox(schemeType: string, schemeVersion: number): Uint8Array {
  return createBox(
    "schm",
    concat(4, strToUtf8(schemeType), itobe4(schemeVersion))
  );
}
...
function createSAIOBox(
  ...
  return createBox(
    ...
    concat(
      ...
      [0, 0, 0, 1], // ??
      itobe4(
        mfhd.length + tfhd.length + tfdt.length + trun.length + 8 + 8 + 8 + 8
      )
    )
  );
}
...
function createTKHDBox(
  ...
  return createBox(
    ...
    concat(
      itobe4(1 + 2 + 4),
      8, // we assume track is enabled,
      // in media and in preview.
      itobe4(trackId),
      20, // we assume trackId = 1;
      ...
      [64, 0, 0, 0], // ??
      itobe2(width),
      2, // width (TODO handle fixed)
      itobe2(height),
      2 // height (TODO handle fixed)
    )
  );
}
