...
import Manifest, { IMetaPlaylistPrivateInfos, ISegment } from "../../manifest";
import parseMetaPlaylist, {
  IParserResponse as IMPLParserResponse,
} from "../../parsers/manifest/metaplaylist";
import { ICdnMetadata, IParsedManifest } from "../../parsers/manifest/types";
 ...
 * @returns {Object}
 */
function getMetaPlaylistPrivateInfos(
  segment: ISegment
): IMetaPlaylistPrivateInfos {
  const { privateInfos } = segment;
  if (privateInfos?.metaplaylistInfos === undefined) {
    throw new Error(
      "MetaPlaylist: Undefined transport for content for metaplaylist."
    );
  }
...
export default function (options: ITransportOptions): ITransportPipelines {
      ...
      function handleParsedResult(
        ...
        const loaderProms = parsedValue.ressources.map((resource) => {
          ...
          function loadSubManifest() {
            ...
            return transport.manifest.loadManifest(
              resource.url,
              manOpts,
              cancelSignal
            );
        ...
        return Promise.all(loaderProms).then((parsedReqs) => {
          ...
          return handleParsedResult(
            parsedResult.value.continue(loadedRessources)
          );
        });
      }
    },
  ...
  function offsetTimeInfos(
    ...
    const offsetedChunkInfos =
      chunkInfos === null ? null : objectAssign({}, chunkInfos);
  ...
  const audioPipeline = {
    ...
    parseSegment(
      ...
      const ogContext = getOriginalContext(context);
      const parsed = audio.parseSegment(
        loadedSegment,
        ogContext,
        initTimescale
      );
  ...
  const videoPipeline = {
    ...
    parseSegment(
      ...
      const { video } = getTransportPipelinesFromSegment(segment);
      const ogContext = getOriginalContext(context);
      const parsed = video.parseSegment(
        loadedSegment,
        ogContext,
        initTimescale
      );
