...
import log from "../../../log";
import { getIndexSegmentEnd, IIndexSegment } from "./index_helpers";
  ...
  for (let i = prevTimelineLength - 1; i >= 0; i--) {
    ...
    if (currStart === newIndexStart) {
      ...
      if (currElt.start + currElt.duration > newIndexStart) {
        ...
        currElt.repeatCount === undefined ||
        currElt.repeatCount <= 0
      ) {
      ...
      const eltLastTime =
        currElt.start + currElt.duration * (currElt.repeatCount + 1);
      ...
      const newCurrRepeat =
        (newIndexStart - currElt.start) / currElt.duration - 1;
      if (
        newCurrRepeat % 1 === 0 &&
        currElt.duration === newTimeline[0].duration
      ) {
