...
describe("Manifest parser index helpers", () => {
  describe("fromIndexTime", () => {
    it("should convert the given index time to a media time", () => {
      expect(
        fromIndexTime(10000, { timescale: 1, indexTimeOffset: 1000 })
      ).toBe(9000);
      expect(
        fromIndexTime(10000, { timescale: 10, indexTimeOffset: 1000 })
      ).toBe(900);
    });
  });
  describe("toIndexTime", () => {
    it("should convert the given media time to an index time", () => {
      expect(toIndexTime(900, { timescale: 1, indexTimeOffset: 100 })).toBe(
        1000
      );
      expect(toIndexTime(900, { timescale: 10, indexTimeOffset: 1000 })).toBe(
        10000
      );
    });
  });
