...
export type IChildrenParser =
  (node : number, attrPtr : number, attrLen : number) => void;

/**
 * Maintains a stack of node parsers, to easily parse the very hierarchical MPDs.
 *
...
export default class ParsersStack {
  ...
  constructor() {
    ...
    this._stack = [{ nodeId: null, children: noop }];
  }
  ...
  public popIfCurrent(idToPop : number) : void {
    ...
    this._stack.pop();
    const { nodeId, children } = this._stack[this._stack.length - 1];
    this._currentNodeId = nodeId;
