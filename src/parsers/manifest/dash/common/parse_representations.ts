...
import { IContentProtections, IParsedRepresentation } from "../../types";
import {
...
export default function parseRepresentations(
  ...
  for (const representation of representationsIR) {
    ...
    let representationID =
      representation.attributes.id != null
    ...
    const unsafelyBaseOnPreviousRepresentation =
      context.unsafelyBaseOnPreviousAdaptation?.getRepresentation(
        representationID
      ) ?? null;

    const inbandEventStreams = combineInbandEventStreams(
      representation,
      adaptation
    );
    ...
    if (contentProtectionsIr.length > 0) {
      const contentProtections =
        contentProtectionsIr.reduce<IContentProtections>(
          (acc, cp) => {
            ...
            if (
              cp.attributes.keyId !== undefined &&
              cp.attributes.keyId.length > 0
            ) {
...
export interface IRepresentationContext
  extends IInheritedRepresentationIndexContext {
