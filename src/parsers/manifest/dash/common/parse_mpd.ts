...
import parsePeriods, { IXLinkInfos } from "./parse_periods";
import resolveBaseURLs, { IResolvedBaseUrl } from "./resolve_base_urls";
...
export default function parseMpdIr(
  ...
  if (xlinksToLoad.length === 0) {
    return parseCompleteIntermediateRepresentation(
      mpdIR,
      args,
      warnings,
      xlinkInfos
  ...
  return {
    ...
    value: {
      ...
      continue: function continueParsingMPD(
        loadedRessources: ILoadedXlinkData[]
      ) {
...
function parseCompleteIntermediateRepresentation(
  ...
  const { externalClockOffset: clockOffset, unsafelyBaseOnPreviousManifest } =
    args;
  ...
  if (!isDynamic) {
    ...
    if (parsedPeriods[parsedPeriods.length - 1] !== undefined) {
      ...
      if (
        ...
        lastPeriodEnd < finalMaximumSafePosition
      ) {
