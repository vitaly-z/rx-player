...
import parseSElement, { IParsedS } from "./parse_s_element";
...
export default function constructTimelineFromElements(
  ...
  for (let i = 0; i < initialTimeline.length; i++) {
    ...
    const nextItem =
      initialTimeline[i + 1] === undefined ? null : initialTimeline[i + 1];
    const timelineElement = convertElementsToIndexSegment(
      item,
      previousItem,
      nextItem
    );
