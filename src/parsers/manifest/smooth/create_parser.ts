...
function createSmoothStreamingParser(
    ...
    switch (streamType) {
      case "audio": {
        ...
        if (
          ...
          log.warn(
            "Smooth parser: Unsupported audio codec. Ignoring quality level."
          );
          return null;
        }
        ...
        return {
          ...
          packetSize:
            packetSize !== undefined ? parseInt(packetSize, 10) : packetSize,
          samplingRate:
      ...
      case "video": {
        ...
        if (
          ...
          log.warn(
            "Smooth parser: Unsupported video codec. Ignoring quality level."
          );
          return null;
        }
      ...
      default:
        log.error(
          "Smooth Parser: Unrecognized StreamIndex type: " + streamType
        );
        return null;
    }
  }
  ...
  function parseAdaptation(
    args: IAdaptationParserArguments
    ...
    const representations = qualityLevels.map((qualityLevel) => {
      ...
      const representation: IParsedRepresentation = objectAssign(
        {},
        qualityLevel,
        { index: reprIndex, cdnMetadata: [{ baseUrl }], mimeType, codecs, id }
      );
  ...
  function parseFromDocument(
    ...
    const { protections, adaptationNodes } = reduceChildren<{
      ...
      (res, name, node) => {
        switch (name) {
          case "Protection": {
            res.protections.push(
              parseProtectionNode(node, parserOptions.keySystems)
            );
            break;
          }
          case "StreamIndex":
    ...
    if (
      firstVideoAdaptation !== undefined ||
      firstAudioAdaptation !== undefined
    ) {
      const firstTimeReferences: number[] = [];
      const lastTimeReferences: number[] = [];

      if (firstVideoAdaptation !== undefined) {
        const firstVideoRepresentation =
          firstVideoAdaptation.representations[0];
      ...
      if (firstAudioAdaptation !== undefined) {
        const firstAudioRepresentation =
          firstAudioAdaptation.representations[0];
      ...
      periods: [
        {
          adaptations,
          duration:
            periodEnd !== undefined ? periodEnd - periodStart : duration,
          end: periodEnd,
