...
export default class SharedSmoothSegmentTimeline {
  ...
  constructor(args: ISharedSmoothSegmentTimelineArguments) {
    const { timeline, timescale, timeShiftBufferDepth, manifestReceivedTime } =
      args;
  ...
  public refresh(): void {
    ...
    const timeSinceLastRealUpdate =
      (performance.now() - this.validityTime) / 1000;
    const lastPositionEstimate =
      timeSinceLastRealUpdate +
  ...
  public replace(newSmoothTimeline: SharedSmoothSegmentTimeline): void {
    ...
    this._initialScaledLastPosition =
      newSmoothTimeline._initialScaledLastPosition;
    ...
    for (let i = 0; i < oldTimeline.length; i++) {
      ...
      if (oldEnd > newEnd) {
        ...
        if (
          rangeDuration < 0 ||
          rangeDuration % oldTimelineRange.duration !== 0
        ) {
          return;
        }
  ...
  public addPredictedSegments(
    ...
    if (currentSegment.privateInfos?.smoothMediaSegment === undefined) {
      log.warn(
        "Smooth Parser: should only encounter SmoothRepresentationIndex"
      );
      return;
    }
