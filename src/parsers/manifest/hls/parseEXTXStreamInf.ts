 ...
 * @returns {Object|null}
 */
export default function parseEXTXStreamInf(
  line : string
) : IParsedXStreamInfLine | null {
  ...
  while (remainingStr.length > 0) {
    ...
    if (indexOfComma === 0) {
      ...
      if (nextEqual > 0) { // there's nothing in a variant without a value
        const key = remainingStr.substring(0, nextEqual);
        const isStringValue = remainingStr[nextEqual + 1] === "\"";
        const startOfValue = isStringValue ? nextEqual + 2 :
                                             nextEqual + 1;
        const endOfValue = remainingStr.substring(startOfValue)
                             .indexOf(isStringValue ? "\"" :
                                                      ",") + startOfValue;
        const value = remainingStr.substring(startOfValue, endOfValue);
        remainingStr = remainingStr.substring(isStringValue ? endOfValue + 2 :
                                                              endOfValue + 1);
        switch (key) {
          case "BANDWIDTH":
