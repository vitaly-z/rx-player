...
export default class HLSRepresentationIndex implements IRepresentationIndex {
  ...
  getInitSegment() : ISegment | null {
    ...
    return { id: "init",
             ...
             duration: 0,
             timescale: 1,
             range: byteRange !== undefined ? translateHLSByteRange(byteRange) :
                                              undefined };
  }
  ...
  getSegments(up : number, duration : number) : ISegment[] {
    ...
    for (let i = 0; i < this._timeline.length; i++) {
      ...
      if (segment.time >= to) {
        ...
        let j = i + 1;
        while (j < this._timeline.length &&
               to > this._timeline[j].time)
        {
          j++;
        }
 ...
 */
function updateHLSTimeline(
  oldTimeline : ISegment[],
  newTimeline : ISegment[]
) : void {
  ...
  for (let i = prevTimelineLength - 1; i >= 0; i--) {
    ...
    if (currStart === newStart) {
      ...
      return;
    } else if (currStart < newStart) { // first to be before
