...
import parseExtXStart, {
  IExtXStart,
} from "./parseExtXStart";
import parseEXTXStreamInf, {
  IParsedXStreamInfLine,
} from "./parseEXTXStreamInf";

export type IXStreamInf = IParsedXStreamInfLine &
                          { defaultURI? : string };
...
export interface IMasterPlaylistIR {
  ...
  startInfo? : IExtXStart;
}

export default function createMasterPlaylistIR(
  playlist : string
) : IMasterPlaylistIR {
  const newLineChar = /\r\n|\n|\r/g;
  ...
  for (let i = 0; i < linified.length; i++) {
    ...
    if (lineType === M3U8LineType.Tag) {
      ...
      if (lastVariant === undefined) {
        ...
        log.warn("HLS Parser: More than one URI associated to a variant",
                 line,
