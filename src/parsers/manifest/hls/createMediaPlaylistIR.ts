 ...
 * @returns {Object}
 */
export default function createMediaPlaylistIR(
  playlist : string
) : IMediaPlaylistIR {
  const newLineChar = /\r\n|\n|\r/g;
  ...
  for (let i = 0; i < linified.length; i++) {
    ...
    if (lineType === M3U8LineType.Tag) {
      ...
      if (startsWith(line, "#EXTINF:")) {
        ...
        // XXX TODO URI=
        const uri = line.substring(16, indexOfComma < 16 ? line.length - 1 :
                                                           indexOfComma - 1);
        const byteRange = indexOfComma >= 0 ?
...
function parseByteRange(
  ...
  const firstPart = line.substring(startIndex,
                                   indexOfSnailLookingChar < 0 ?
                                     line.length :
                                     indexOfSnailLookingChar);
  const secondPart = indexOfSnailLookingChar < 0 ||
                     indexOfSnailLookingChar === line.length - 1 ?
                       undefined :
                       line.substring(indexOfSnailLookingChar + 1, line.length);
  const parsedFirstPart = parseInt(firstPart, 10);
  const parsedSecondPart = secondPart === undefined ? undefined :
                                                      parseInt(secondPart, 10);
  if (isNaN(parsedFirstPart) ||
