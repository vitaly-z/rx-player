  ...
  public start(): void {
    ...
    this._playbackObserver.listen(
      (observation) => {
        ...
        const { buffered, position, readyState, rebuffering, freezing } =
          observation;
        ...
        if (
          ...
          if (this._playbackObserver.getCurrentTime() < seekTo) {
            ...
            this.trigger(
              "warning",
              generateDiscontinuityError(freezePosition, seekTo)
            );
            return;
          }
        }
        ...
        for (let i = this._manifest.periods.length - 2; i >= 0; i--) {
          ...
          if (period.end !== undefined && period.end <= freezePosition) {
            if (
              this._manifest.periods[i + 1].start > freezePosition &&
              ...
              this.trigger(
                "warning",
                generateDiscontinuityError(freezePosition, nextPeriod.start)
              );
              return;
            }
            break;
