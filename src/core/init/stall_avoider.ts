    ...
    map(([tick, discontinuitiesStore]) => {
      ...
      if (stalledPosition !== null) {
        ...
        if (skippableDiscontinuity !== null) {
          console.error("IN STREAM-DETECTED DISCONTINUITY", tick.event, tick.readyState);
      ...
      if (isPlaybackStuck(position,
        ...
        console.error("IN BROWSER SEEK DISCONTINUITY", tick.event, tick.readyState);
      ...
      if (nextBufferRangeGap < BUFFER_DISCONTINUITY_THRESHOLD) {
        console.error("IN MINI-HOLE DISCONTINUITY", tick.event, tick.readyState);
