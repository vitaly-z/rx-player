...
export default class WorkerContentInitializer extends ContentInitializer {
  ...
  private _preparedContentId : string | null;
   ...
   */
  public prepare(): void {
    if (this._preparedContentId !== null || this._initCanceller.isUsed()) {
      return;
    }
    this._preparedContentId = generateContentId();
    ...
    sendMessage(worker,
                { type: "prepare",
                  value: { contentId: this._preparedContentId,
  ...
  public updateContentUrls(urls : string[] | undefined, refreshNow : boolean) : void {
    if (this._preparedContentId === null) {
      return;
    }
    sendMessage(this._settings.worker, { type: "updateContentUrls",
                                         value: { contentId: this._preparedContentId,
                                                  urls,
                                                  refreshNow } });
  }
  ...
  public start(
    ...
    const onmessage = (_msg: MessageEvent) => {
      const msgData = _msg.data as unknown as IWorkerMessage;
      switch (msgData.type) {
        case "media-source": {
          const handle = msgData.value;
          const listenCanceller = new TaskCanceller();
          listenCanceller.linkToSignal(this._initCanceller.signal);
        ...
        case "warning":
          this.trigger("warning", formatError(msgData.value));
          break;

        case "error":
          this._onFatalError(formatError(msgData.value));
          break;
        ...
        case "ready-to-start": {
          const loadedManifest = msgData.value.manifest;
          const listenCanceller = new TaskCanceller();
          listenCanceller.linkToSignal(this._initCanceller.signal);
  ...
  private _initializeContentDecryption(
    ...
    contentDecryptor.addEventListener("stateChange", (state) => {
      if (state === ContentDecryptorState.WaitingForAttachment) {
        ...
        mediaSourceStatusListenerCanceller.linkToSignal(listenCanceller.signal);
        mediaSourceStatus.onUpdate((currStatus) => {
