...
import { getKeySystemConfiguration, IContentProtection } from "../decrypt";
...
import getInitialTime, { IInitialTimeOptions } from "./utils/get_initial_time";
...
export default class MediaSourceContentInitializer extends ContentInitializer {
        ...
        (drmStatus, stopListeningToDrmUpdates) => {
          ...
          openMediaSource(mediaElement, mediaSourceCanceller.signal)
            .then((mediaSource) => {
              ...
              if (
                lastDrmStatus.initializationState.type === "awaiting-media-link"
              ) {
                lastDrmStatus.initializationState.value.isMediaLinked.setValue(
                  true
                );
                drmInitRef.onUpdate(
                  (newDrmStatus, stopListeningToDrmUpdatesAgain) => {
                    if (
                      newDrmStatus.initializationState.type === "initialized"
                    ) {
  ...
  private async _onInitialMediaSourceReady(
    ...
    const representationEstimator =
      AdaptiveRepresentationSelector(adaptiveOptions);
  ...
  private _startBufferingOnMediaSource(
    ...
    const initialPeriod =
      manifest.getPeriodForTime(initialTime) ??
    ...
    const segmentBuffersStore = new SegmentBuffersStore(
      mediaElement,
      mediaSource
    );
    ...
    const contentTimeBoundariesObserver =
      this._createContentTimeBoundariesObserver(
        manifest,
        mediaSource,
        streamObserver,
        segmentBuffersStore,
    ...
    function handleStreamOrchestratorCallbacks(): IStreamOrchestratorCallbacks {
      return {
        ...
        streamStatusUpdate(value) {
          ...
          if (
            ...
            if (hasFinishedLoadingLastPeriod) {
              ...
              contentTimeBoundariesObserver.onLastSegmentLoadingResume(
                value.bufferType
              );
            }
          }
        ...
        representationChange: (value) => {
          ...
          contentTimeBoundariesObserver.onRepresentationChange(
            value.type,
            value.period
          );
        },
        ...
        periodStreamCleared: (value) => {
          contentTimeBoundariesObserver.onPeriodCleared(
            value.type,
            value.period
          );
          if (cancelSignal.isCancelled) {
  ...
  private _createContentTimeBoundariesObserver(
    ...
    const mediaDurationUpdater = new MediaDurationUpdater(
      manifest,
      mediaSource
    );
    cancelSignal.register(() => {
