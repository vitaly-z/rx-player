...
export default function createAdaptiveRepresentationSelector(
  ...
  return function getEstimates(
    context: { manifest: Manifest; period: Period; adaptation: Adaptation },
...
function getEstimateReference(
  ...
  function createEstimateReference(
     ...
     */
    const guessBasedChooser = new GuessBasedChooser(
      scoreCalculator,
      prevEstimate
    );
    ...
    const innerEstimateRef = createSharedReference<IABREstimate>(
      getCurrentEstimate()
    );
    ...
    function getCurrentEstimate() {
      ...
      const { bandwidthEstimate, bitrateChosen } =
        networkAnalyzer.getBandwidthEstimate(
...
export interface IRepresentationEstimatorThrottlers {
  ...
  >;
  throttleBitrate: Partial<
    Record<IBufferType, IReadOnlySharedReference<number>>
  >;
}
