...
describe("BufferBasedChooser", () => {
  ...
  it("should return the first bitrate if the current bitrate is undefined", () => {
    ...
    const BufferBasedChooser = jest.requireActual(
      "../buffer_based_chooser"
    ).default;
    expect(
      new BufferBasedChooser([]).getEstimate({ bufferGap: 0, speed: 1 })
    ).toEqual(undefined);
    expect(
      new BufferBasedChooser([1, 2, 3]).getEstimate({
        bufferGap: 0,
        speed: 1,
      })
    ).toEqual(1);
    expect(
      new BufferBasedChooser([10, 20]).getEstimate({
        bufferGap: 0,
        speed: 1,
  ...
  it("should go to the next bitrate if the current one is maintainable and we have more buffer than the next level", () => {
    ...
    expect(
      new BufferBasedChooser([10, 20, 40]).getEstimate({
        bufferGap: 16,
        speed: 1,
        currentBitrate: 10,
        currentScore: 1.01,
    ...
    expect(
      new BufferBasedChooser([10, 20, 20, 40]).getEstimate({
        bufferGap: 30,
        speed: 2,
        currentBitrate: 20,
        currentScore: 2.1,
  ...
  it("should stay at the current bitrate if we are currently at the maximum one", () => {
    ...
    expect(
      new BufferBasedChooser([10, 20, 40, 40]).getEstimate({
        bufferGap: 100000000000,
        speed: 1,
        currentBitrate: 40,
        currentScore: 1000000,
