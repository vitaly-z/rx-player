...
import BufferedHistory, { IBufferedHistoryEntry } from "./buffered_history";
...
export default class SegmentInventory {
  ...
  public synchronizeBuffered(buffered: TimeRanges): void {
    ...
    for (let i = 0; i < rangesLength; i++) {
      ...
      // `thisSegment`.
      let lastDeletedSegmentInfos: { end: number; precizeEnd: boolean } | null =
        null;
      ...
      if (
        ...
        let thisSegmentStart = takeFirstSet<number>(
          thisSegment.bufferedStart,
          thisSegment.start
        );
        let thisSegmentEnd = takeFirstSet<number>(
          thisSegment.bufferedEnd,
          thisSegment.end
        );
        const nextRangeStart =
          i < rangesLength - 1 ? buffered.start(i + 1) : undefined;
        while (
          thisSegment !== undefined &&
          ...
          thisSegment = inventory[++inventoryIndex];
          if (thisSegment !== undefined) {
            thisSegmentStart = takeFirstSet<number>(
              thisSegment.bufferedStart,
              thisSegment.start
            );
            thisSegmentEnd = takeFirstSet<number>(
              thisSegment.bufferedEnd,
              thisSegment.end
            );
          }
    ...
    if (thisSegment != null) {
      ...
      const removed = inventory.splice(
        inventoryIndex,
        inventory.length - inventoryIndex
      );
      for (const seg of removed) {
        ...
        if (segmentI.end <= start) {
          ...
          if (segmentI.start === start) {
            if (segmentI.end <= end) {
              ...
              segmentI.precizeStart =
                segmentI.precizeStart && newSegment.precizeEnd;
              return;
            }
          } else {
            if (segmentI.end <= newSegment.end) {
              ...
              segmentI.precizeEnd =
                segmentI.precizeEnd && newSegment.precizeStart;
              i += 2; // Go to segment immediately after newSegment
              while (
                i < inventory.length &&
                inventory[i].start < newSegment.end
  ...
  return (
    ...
    (thisSegment.bufferedEnd === undefined ||
      (thisSegment.bufferedEnd > thisSegment.bufferedStart &&
        Math.abs(
          thisSegment.bufferedEnd - thisSegment.bufferedStart - duration
...
function prettyPrintInventory(inventory: IBufferedChunk[]): string {
  ...
  const encounteredReps: Partial<
    Record<
      string /* Period `id` */,
      Partial<
        Record<string /* Representation `id` */, string /* associated letter */>
      >
    >
  > = {};
  const letters: Array<{
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    if (chunk.bufferedStart !== undefined && chunk.bufferedEnd !== undefined) {
      ...
      if (lastChunk === null) {
        ...
        if (
          (lastChunk.bufferedEnd as number) + roundingError <
          chunk.bufferedStart
        ) {
          str +=
