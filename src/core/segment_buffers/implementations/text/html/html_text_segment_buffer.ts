...
export default class HTMLTextSegmentBuffer extends SegmentBuffer {
  ...
  public pushChunkSync(infos: IPushChunkInfos<unknown>): void {
    ...
    if (startTime !== undefined) {
      ...
      if (cues.length <= 0) {
        log.warn(
          "HTSB: Current text tracks have no cues nor start time. Aborting"
        );
        return;
      }
    ...
    if (endTime !== undefined) {
      ...
      if (cues.length <= 0) {
        log.warn(
          "HTSB: Current text tracks have no cues nor end time. Aborting"
        );
        return;
      }
    ...
    if (end <= start) {
      log.warn(
        "HTSB: Invalid text track appended: ",
  ...
  private _displayCues(elements: HTMLElement[]): void {
    const nothingChanged =
      ...
      this._currentCues.every(
        (current, index) => current.element === elements[index]
      );
...
function assertChunkIsTextTrackSegmentData(
  ...
  if (
    ...
    ((chunk as INativeTextTracksBufferSegmentData).language !== undefined &&
      typeof (chunk as INativeTextTracksBufferSegmentData).language !==
        "string") ||
    ((chunk as INativeTextTracksBufferSegmentData).start !== undefined &&
      typeof (chunk as INativeTextTracksBufferSegmentData).start !==
        "number") ||
