            ...
            case "chunk-complete":
              return initSegmentTimescale$.pipe(
                ...
                map((initTimescale) => {
                  ...
                  return objectAssign({ segment }, parsed);
                }));
    ...
    return request$
      .pipe(mergeMap((evt) : Observable<IDownloadingQueueEvent<T>> => {
        switch (evt.type) {
          ...
          case "chunk":
            const parserResponse = evt.parse(undefined);
            if (parserResponse.segmentType === "init") {
              initSegmentTimescale$.next(parserResponse.initTimescale);
            }
            return observableOf(objectAssign({ segment }, parserResponse));
