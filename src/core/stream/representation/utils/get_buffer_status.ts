...
export default function getBufferStatus(
  ...
  const askedStart =
    playbackObserver.getIsPaused() || playbackObserver.getPlaybackRate() <= 0
  ...
  const prioritizedNeededSegments: IQueuedSegment[] = segmentsToLoad.map(
    (segment) => ({
