 ...
 */
export default class DownloadingQueue<T> extends EventEmitter<
  IDownloadingQueueEvent<T>
> {
  ...
  public getRequestedInitSegment(): ISegment | null {
    return this._initSegmentRequest === null
      ? null
  ...
  public getRequestedMediaSegment(): ISegment | null {
    return this._mediaSegmentRequest === null
      ? null
    ...
    this._downloadQueue.onUpdate(
      (next) => {
        ...
        if (next.initSegment !== null && initSegmentRequest !== null) {
          ...
          next.initSegment?.segment.id === initSegmentRequest?.segment.id
        ) {
          return;
        }
  ...
  private _restartMediaSegmentDownloadingQueue(): void {
    ...
    const recursivelyRequestSegments = (
      ...
      const canceller = new TaskCanceller({
        cancelOn: this._currentCanceller?.signal,
      });
      ...
      const emitChunk = (
        parsed:
          | ISegmentParserParsedInitChunk<T>
