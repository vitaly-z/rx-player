...
import { IPeriodStreamPlaybackObservation } from "../period";
import {
...
export default function ContentTimeBoundariesObserver(
...
) : Observable<IContentDurationUpdateEvent | IStreamWarningEvent> {
  /**
   * Allows to calculate the minimum and maximum playable position on the
   * whole content.
   */
  const mediaTimeBoundariesCalculator = new MediaTimeBoundariesCalculator(manifest);
  ...
  const outOfManifest$ = playbackObserver.observe(true).pipe(
    filterMap<IPeriodStreamPlaybackObservation, IStreamWarningEvent, null>((
      { position, wantedTimeOffset }
    ) => {
      const offsetedPosition = wantedTimeOffset + position;
      if (
        offsetedPosition < mediaTimeBoundariesCalculator.getCurrentMinimumPosition()
      ) {
        ...
        offsetedPosition > mediaTimeBoundariesCalculator.getCurrentMaximumPosition()
      ) {
  ...
  const updateDurationOnManifestUpdate$ = fromEvent(manifest, "manifestUpdate").pipe(
    ...
    tap(() => {
      if (!manifest.isDynamic) {
        const maxPos = mediaTimeBoundariesCalculator.getCurrentMaximumPosition();
      ...
      if (message.type === "adaptationChange") {
        ...
        if (message.value.period.id === lastPeriod?.id) {
          if (message.value.type === "audio") {
            mediaTimeBoundariesCalculator
              .updateLastAudioAdaptation(message.value.adaptation);
            if (!manifest.isDynamic) {
              contentDuration.setValue(
                mediaTimeBoundariesCalculator.getCurrentMaximumPosition()
              );
            }
...
class MediaTimeBoundariesCalculator {
  ...
  public getCurrentMinimumPosition() : number {
    const minimumPosition = this._manifest.getMinimumSafePosition();
