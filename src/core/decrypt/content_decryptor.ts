...
export default class ContentDecryptor extends EventEmitter<IContentDecryptorEvent> {
  ...
  public attach(): void {
    if (this._stateData.state !== ContentDecryptorState.WaitingForAttachment) {
      throw new Error(
        "`attach` should only be called when " +
          "in the WaitingForAttachment state"
      );
    } else if (this._wasAttachCalled) {
      log.warn(
        "DRM: ContentDecryptor's `attach` method called more than once."
      );
      return;
    }
  ...
  public onInitializationData(initializationData: IProtectionData): void {
    ...
    this._processInitializationData(
  ...
  private async _processInitializationData(
      ...
      if (firstCreatedSession !== undefined) {
        ...
        if (keyIds === undefined) {
          if (initializationData.content === undefined) {
            log.warn(
              "DRM: Unable to fallback from a non-decipherable quality."
            );
          } else {
        ...
        if (initializationData.content !== undefined) {
          if (log.hasLevel("DEBUG")) {
            const hexKids = keyIds.reduce(
              (acc, kid) => `${acc}, ${bytesToHex(kid)}`,
              ""
            );
    ...
    if (isNullOrUndefined(options.persistentLicenseConfig)) {
      ...
      log.warn(
        'DRM: Cannot create "persistent-license" session: not supported'
      );
      wantedSessionType = "temporary";
    } else {
      ...
      mediaKeySystemAccess.keySystem,
      {
        ...
        onError: (err: unknown): void => {
          ...
          if (initializationData.content !== undefined) {
            ...
            log.info(
              "DRM: blacklisting Representations based on " + "protection data."
            );
    ...
    if (sessionRes.type === MediaKeySessionLoadingType.Created) {
      ...
      try {
        ...
        // or being closed
        const entry =
          stores.loadedSessionsStore.getEntryForSession(mediaKeySession);
  ...
  private _tryToUseAlreadyCreatedSession(
    ...
    const blacklistedSessionError =
      compatibleSessionInfo.blacklistedSessionError;
 ...
 * @param {Object} period
 */
function addKeyIdsFromPeriod(set: Set<Uint8Array>, period: Period) {
