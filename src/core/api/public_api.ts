...
class Player extends EventEmitter<IPublicAPIEvent> {
  ...
  constructor(options : IConstructorOptions = {}) {
    ...
    this.version = /* PLAYER_VERSION */"3.29.0-canal.2022111001";
    this.log = log;
    this.state = "STOPPED";
  ...
  private _priv_initializeContentPlayback(options : IParsedLoadVideoOptions) : void {
    ...
    const loadedStateUpdates$ = observableCombineLatest([
      ...
      map(([stalledStatus]) =>
        getLoadedContentState(videoElement, stalledStatus)
      )
    );
...
Player.version = /* PLAYER_VERSION */"3.29.0-canal.2022111001";
