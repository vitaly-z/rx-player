...
import PlaybackObserver, { IPlaybackObservation } from "./playback_observer";
/* eslint-disable-next-line max-len */
import MediaElementTracksStore from "./track_management/media_element_tracks_store";
import TracksStore, { ITMPeriodObject } from "./track_management/tracks_store";
import {
...
class Player extends EventEmitter<IPublicAPIEvent> {
  ...
  constructor(options: IConstructorOptions = {}) {
    ...
    this._priv_speed = createSharedReference(
    ...
    this._priv_bufferOptions = {
      wantedBufferAhead: createSharedReference(
        wantedBufferAhead,
        this._destroyCanceller.signal
      ),
      maxBufferAhead: createSharedReference(
        maxBufferAhead,
        this._destroyCanceller.signal
      ),
      maxBufferBehind: createSharedReference(
        maxBufferBehind,
        this._destroyCanceller.signal
      ),
      maxVideoBufferSize: createSharedReference(
        maxVideoBufferSize,
        this._destroyCanceller.signal
      ),
    };
  ...
  private _priv_initializeContentPlayback(
    ...
    let mediaElementTracksStore: MediaElementTracksStore | null = null;
    if (!isDirectFile) {
  ...
  getCurrentBufferGap(): number {
    ...
    const videoElement = this.videoElement;
    const bufferGap = getLeftSizeOfRange(
  ...
  __priv_getSegmentBufferContent(
    ...
    const segmentBufferStatus =
      this._priv_contentInfos.segmentBuffersStore.getStatus(bufferType);
  ...
  private _priv_onManifestReady(
    ...
    manifest.addEventListener(
      ...
      (updates) => {
        ...
        const currentPeriod =
          this._priv_contentInfos?.currentPeriod ?? undefined;
        const tracksStore = this._priv_contentInfos?.tracksStore;
        ...
        for (const update of updates.updatedPeriods) {
          if (update.period.id === currentPeriod.id) {
            if (
              ...
              const audioTracks =
                tracksStore.getAvailableAudioTracks(periodRef);
              ...
              const videoTracks =
                tracksStore.getAvailableVideoTracks(periodRef);
              this._priv_triggerEventIfNotStopped(
  ...
  private _priv_onActivePeriodChanged(
    ...
    const audioRepresentation =
      this._priv_getCurrentRepresentations()?.audio ?? null;
    this._priv_triggerEventIfNotStopped(
    ...
    const videoRepresentation =
      this._priv_getCurrentRepresentations()?.video ?? null;
    this._priv_triggerEventIfNotStopped(
  ...
  private _priv_onPeriodStreamReady(
    ...
    switch (type) {
      ...
      case "text":
        if (isNullOrUndefined(tracksStore)) {
          log.error(
            `API: TracksStore not instanciated for a new ${type} period`
          );
  ...
  private _priv_onAdaptationChange(
    contentInfos: IPublicApiContentInfos,
    {
      ...
      adaptation,
      period,
    }: { type: IBufferType; adaptation: Adaptation | null; period: Period }
  ): void {
    if (contentInfos.contentId !== this._priv_contentInfos?.contentId) {
      return; // Event for another content
    }
  ...
  private _priv_onRepresentationChange(
    ...
    if (
      ...
      const cancelSignal =
        this._priv_contentInfos.currentContentCanceller.signal;
      if (type === "video") {
        this._priv_triggerEventIfNotStopped(
          "videoRepresentationChange",
          representation,
  ...
  private _priv_triggerPositionUpdate(
    ...
    if (
      (!isDirectFile && manifest === null) ||
      isNullOrUndefined(observation)
    ) {
      return;
    }

    const maximumPosition =
      manifest !== null ? manifest.getMaximumSafePosition() : undefined;
  ...
  private _priv_getCurrentRepresentations(): Partial<
  ...
  private _priv_triggerEventIfNotStopped<
    TEventName extends keyof IPublicAPIEvent
  >(
    evt: TEventName,
