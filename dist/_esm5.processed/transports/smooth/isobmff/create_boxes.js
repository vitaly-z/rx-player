...
import { be2toi, be4toi, concat, hexToBytes, itobe2, itobe4, itobe8, strToBytes, } from "../../../utils/byte_parsing";
/**
 * @param {Number} width
 * @param {Number} height
...
function createHDLRBox(type) {
    ...
    return createBox("hdlr", concat(8, strToBytes(name), 12, strToBytes(handlerName), 1 // handler name is C-style string (0 terminated)
    ));
}
/**
...
function createPSSHBox(systemId, privateData, keyIds) {
    ...
    return createBox("pssh", concat([version, 0, 0, 0], hexToBytes(_systemId), kidList, itobe4(privateData.length), privateData));
}
/**
...
function createSTSDBox(reps) {
    ...
    return createBox("stsd", concat.apply(void 0, arrBase.concat(reps)));
}
/**
 * @param {Number} width
 * @param {Number} height
