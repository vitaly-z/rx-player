...
function getRange(timeRanges, time) {
    ...
    for (var i = timeRanges.length - 1; i >= 0; i--) {
        var start = timeRanges.start(i) + ((_a = window.offset) !== null && _a !== void 0 ? _a : 0);
        if (time >= start) {
            var end = timeRanges.end(i) + ((_b = window.offset) !== null && _b !== void 0 ? _b : 0);
            if (time < end) {
                return { start: start,
                    end: end };
            }
...
export { convertToRanges, excludeFromRanges, getInnerAndOuterTimeRanges, getLeftSizeOfRange, getNextRangeGap, getPlayedSizeOfRange, getRange, getSizeOfRange, insertInto, isAfter, isBefore, isTimeInTimeRanges, isTimeInRange, isTimeInRanges, keepRangeIntersection, mergeContiguousRanges, removeEmptyRanges, };
