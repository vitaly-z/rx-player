...
export default function parsePeriods(periodsIR, contextInfos) {
    ...
    var _loop_1 = function (i) {
        ...
        var periodInfos = { aggressiveMode: contextInfos.aggressiveMode, availabilityTimeOffset: availabilityTimeOffset, baseURLs: periodBaseURLs, manifestBoundsCalculator: manifestBoundsCalculator, end: periodEnd, isDynamic: isDynamic, manifestProfiles: contextInfos.manifestProfiles, receivedTime: receivedTime, segmentTemplate: periodIR.children.segmentTemplate,
            start: periodStart, timeShiftBufferDepth: timeShiftBufferDepth, unsafelyBaseOnPreviousPeriod: unsafelyBaseOnPreviousPeriod };
        var adaptations = parseAdaptationSets(periodIR.children.adaptations, periodInfos);
        var streamEvents = generateStreamEvents(periodIR.children.eventStreams, periodStart);
 ...
 */
function generateStreamEvents(baseIr, periodStart) {
    var _a;
    var res = [];
    for (var i = 0; i < baseIr.length; i++) {
        var eventStreamIr = baseIr[i];
        var _b = eventStreamIr.attributes, _c = _b.schemeIdUri, schemeIdUri = _c === void 0 ? "" : _c, _d = _b.timescale, timescale = _d === void 0 ? 1 : _d;
        for (var j = 0; j < eventStreamIr.children.events.length; j++) {
            var eventIr = eventStreamIr.children.events[j];
            if (eventIr.eventStreamData !== undefined) {
                var start = (((_a = eventIr.presentationTime) !== null && _a !== void 0 ? _a : 0) / timescale) + periodStart;
                var end = eventIr.duration === undefined ?
                    undefined :
                    start + (eventIr.duration / timescale);
                var element = eventIr.eventStreamData instanceof Element ?
                    eventIr.eventStreamData :
                    new DOMParser()
                        .parseFromString(utf8ToStr(new Uint8Array(eventIr.eventStreamData)), "application/xml").documentElement;
