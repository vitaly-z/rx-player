 ...
 * @returns {Observable}
 */
export default function BufferBasedChooser(update$, bitrates) {
    var levelsMap = getBufferLevels(bitrates);
    log.debug("ABR: Steps for buffer based chooser.", levelsMap.map(function (l, i) { return ({ bufferLevel: l, bitrate: bitrates[i] }); }));
    return update$.pipe(map(function (playbackObservation) {
