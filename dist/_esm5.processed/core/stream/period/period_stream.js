 ...
 */
import { catchError, concat as observableConcat, defer as observableDefer, EMPTY, ignoreElements, map, mapTo, merge as observableMerge, mergeMap, of as observableOf, ReplaySubject, startWith, switchMap, } from "rxjs";
...
export default function PeriodStream(_a) {
    ...
    return adaptation$.pipe(switchMap(function (adaptation, switchNb) {
        ...
        if (adaptation === null) { // Current type is disabled for that Period
            ...
            return observableConcat(cleanBuffer$.pipe(mapTo(EVENTS.adaptationChange(bufferType, null, period))), createEmptyStream(playbackObserver, wantedBufferAhead, bufferType, { period: period }));
        }
        if (SegmentBuffersStore.isNative(bufferType) &&
