...
import { catchError, distinctUntilChanged, filter, ignoreElements, map, multicast, share, startWith, tap, } from "rxjs/operators";
...
export default function AdaptationStream(_a) {
    ...
    var terminateCurrentStream$ = new Subject();
    // use ABRManager for choosing the Representation
    var streamEvents$ = new Subject();
    var requestsEvents$ = new Subject();
    var abrEvents$ = observableMerge(streamEvents$, requestsEvents$);
    ...
    var adaptationStream$ = observableMerge(newRepresentation$
        ...
        return observableConcat(representationChange$, createRepresentationStream(representation))
    ...
    newRepresentation$.pipe(map(function (estimate, i) {
     ...
     * @returns {Observable}
     */
    function createRepresentationStream(representation) {
