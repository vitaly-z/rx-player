 ...
 */
import { catchError, concat as observableConcat, defer as observableDefer, distinctUntilChanged, EMPTY, exhaustMap, filter, map, merge as observableMerge, mergeMap, of as observableOf, share, take, tap, } from "rxjs";
...
export default function AdaptationStream(_a) {
    ...
    var _b = createRepresentationEstimator(content, abrManager, playbackObserver.observe(true)), estimator$ = _b.estimator$, abrFeedbacks$ = _b.abrFeedbacks$;
    ...
    var segmentFetcher = segmentFetcherCreator
        ...
        onRequestBegin: function (value) {
            abrFeedbacks$.next({ type: "requestBegin", value: value });
        },
        onRequestEnd: function (value) {
            abrFeedbacks$.next({ type: "requestEnd", value: value });
        },
        onProgress: function (value) {
            abrFeedbacks$.next({ type: "progress", value: value });
        },
        onMetrics: function (value) {
            abrFeedbacks$.next({ type: "metrics", value: value });
        },
    });
    /**
