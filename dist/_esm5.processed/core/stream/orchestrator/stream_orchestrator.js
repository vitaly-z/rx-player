...
export default function StreamOrchestrator(content, clock$, abrManager, segmentBuffersStore, segmentFetcherCreator, options) {
    ...
    var garbageCollectors = new WeakMapMemory(function (segmentBuffer) {
        ...
        return BufferGarbageCollector({
            ...
            maxBufferBehind$: maxBufferBehind$.pipe(map(function (val) { return Math.min(val, defaultMaxBehind); })),
            maxBufferAhead$: maxBufferAhead$.pipe(map(function (val) { return Math.min(val, defaultMaxAhead); })),
        });
    });
    ...
    return observableMerge.apply(void 0, __spreadArray(__spreadArray([], streamsArray), [activePeriodChanged$,
        endOfStream$,
        outOfManifest$]));
    /**
