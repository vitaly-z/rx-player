    ...
    function loadSegmentsFromQueue() {
        var requestNextSegment$ = observableDefer(function () {
            ...
            return request$
                ...
                switch (evt.type) {
                    case "warning":
                        return observableOf({ type: "retry", segment: segment, error: evt.value });
                    case "chunk-complete":
                    ...
                    case "chunk":
                        ...
                        return observableOf({ type: "parsed", segment: segment, payload: parsed });
                    case "ended":
    ...
    function onParsedChunk(evt) {
        ...
        if (parsed.segmentType === "init") {
            ...
            var pushEvent$ = pushInitSegment({ clock$: clock$, content: content, segment: evt.segment,
                segmentData: parsed.initializationData, segmentBuffer: segmentBuffer });
            ...
            var inbandEvents$ = inbandEvents !== undefined &&
                ...
                observableOf({ type: "inband-events",
                    value: inbandEvents }) :
                EMPTY;
            return observableConcat(segmentEncryptionEvent$, manifestRefresh$, inbandEvents$, pushMediaSegment({ clock$: clock$, content: content, initSegmentData: initSegmentData, parsedSegment: parsed,
