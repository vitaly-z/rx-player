...
import { BehaviorSubject, combineLatest as observableCombineLatest, concat as observableConcat, defer as observableDefer, EMPTY, merge as observableMerge, of as observableOf, Subject, } from "rxjs";
import { ignoreElements, mergeMap, share, startWith, take, takeWhile, withLatestFrom, } from "rxjs/operators";
import log from "../../../log";
...
export default function RepresentationStream(_a) {
    ...
    var lastSegmentQueue$ = new BehaviorSubject({ initSegment: null,
        segmentQueue: [] });
    ...
    function onParsedChunk(evt) {
        if (evt.segmentType === "init") {
            ...
            var initEncEvt$ = !hasSentEncryptionData &&
                ...
                EMPTY;
            var pushEvent$ = pushInitSegment({ clock$: clock$, content: content, segment: evt.segment,
            ...
            var pushMediaSegment$ = pushMediaSegment({ clock$: clock$, content: content, initSegmentData: initSegmentData, parsedSegment: evt,
