...
function getMediaInfos(mediaElement, currentState) {
    ...
    return { bufferGap: getLeftSizeOfRange(buffered, currentTime), buffered: buffered, currentRange: getRange(buffered, currentTime), currentTime: currentTime,
        duration: duration,
        ended: ended,
        paused: paused,
...
function getStalledStatus(prevTimings, currentTimings, _a) {
     ...
     */
    var canFreeze = ((!paused &&
        !prevPaused) ||
        (readyState >= 1 &&
            prevReadyState >= 1)) &&
        (playbackRate !== 0 && prevPlaybackRate !== 0) &&
        currentRange !== null;
    var isFreezing = (currentTime === prevTime) && canFreeze;
    var shouldStall;
    var shouldUnstall;
    if (withMediaSource) {
        if (canStall && (isFreezing || ((bufferGap <= (lowLatencyMode ? STALL_GAP.LOW_LATENCY : STALL_GAP.DEFAULT) ||
            bufferGap === Infinity || readyState === 1)))) {
            shouldStall = true;
        }
        else if (prevStalled !== null &&
            (prevStalled.reason !== "freezing" || !isFreezing) && (readyState > 1 &&
            ((bufferGap < Infinity &&
    ...
    if (shouldUnstall === true) {
        ...
        if (isFreezing) {
            ...
            (prevStalled === null || prevStalled === void 0 ? void 0 : prevStalled.reason) === "seeking") {
            reason = "seeking";
        }
