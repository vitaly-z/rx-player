...
function getMediaInfos(mediaElement, currentState) {
    ...
    var newCurrentTime = currentTime + ((_a = window.offset) !== null && _a !== void 0 ? _a : 0);
    var currentRange = getRange(buffered, newCurrentTime);
    return { bufferGap: currentRange !== null ? currentRange.end - newCurrentTime :
            // TODO null/0 would probably be
            // more appropriate
            Infinity, buffered: buffered,
        currentRange: currentRange, position: newCurrentTime, duration: duration,
        ended: ended,
        paused: paused,
...
function getStalledStatus(prevTimings, currentTimings, _a) {
    ...
    var currentState = currentTimings.state, currentTime = currentTimings.position, bufferGap = currentTimings.bufferGap, currentRange = currentTimings.currentRange, duration = currentTimings.duration, paused = currentTimings.paused, readyState = currentTimings.readyState, ended = currentTimings.ended;
    var prevStalled = prevTimings.stalled, prevState = prevTimings.state, prevTime = prevTimings.position;
...
function createClock(mediaElement, options) {
    return observableDefer(function () {
        var lastTimings = objectAssign(getMediaInfos(mediaElement, "init"), { stalled: null, getCurrentTime: function () { var _a; return mediaElement.currentTime + ((_a = window.offset) !== null && _a !== void 0 ? _a : 0); } });
        function getCurrentClockTick(state) {
            ...
            var timings = objectAssign({}, { stalled: stalledState, getCurrentTime: function () {
                    var _a;
                    return mediaElement.currentTime +
                        ((_a = window.offset) !== null && _a !== void 0 ? _a : 0);
                } }, mediaTimings);
...
function prettyPrintBuffered(buffered, currentTime) {
    ...
    for (var i = 0; i < buffered.length; i++) {
        var start = buffered.start(i) + ((_a = window.offset) !== null && _a !== void 0 ? _a : 0);
        var end = buffered.end(i) + ((_b = window.offset) !== null && _b !== void 0 ? _b : 0);
        ...
        if (i < buffered.length - 1) {
            var nextStart = buffered.start(i + 1) + ((_c = window.offset) !== null && _c !== void 0 ? _c : 0);
