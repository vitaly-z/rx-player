...
var Player = /** @class */ (function (_super) {
    ...
    function Player(options) {
        ...
        _this.version = /* PLAYER_VERSION */ "3.26.2+fixed_ttml_subs+session_remove_before_close-3";
        _this.log = log;
        _this.state = "STOPPED";
        ...
        _this._priv_isPlaying = createSharedReference(false);
        _this._priv_speed = createSharedReference(videoElement.playbackRate);
        _this._priv_preferTrickModeTracks = false;
        _this._priv_contentLock = createSharedReference(false);
    ...
    Player.prototype._priv_initializeContentPlayback = function (options) {
        ...
        if (!isDirectFile) {
            ...
            if (this._priv_throttleWhenHidden) {
                if (!relyOnVideoVisibilityAndSize) {
                    ...
                    throttlers.throttle = {
                        video: isPageActive().pipe(map(function (active) { return active ? Infinity :
            ...
            var directfileInit$ = features.directfile.initDirectFile({ autoPlay: autoPlay, keySystems: keySystems, mediaElement: videoElement,
                speed: this._priv_speed, playbackObserver: playbackObserver, startAt: startAt, url: url })
                .pipe(takeUntil(stopContent$));
        ...
        var stalled$ = playback$.pipe(filter(function (evt) { return evt.type === "stalled" ||
            evt.type === "unstalled"; }), map(function (x) { return x.value; }), distinctUntilChanged(function (prevStallReason, currStallReason) {
        ...
        loaded$.pipe(take(1), map(function () { return PLAYER_STATES.LOADED; })), observableMerge(loadedStateUpdates$
            .pipe(
        ...
        // when reloading
        reloading$.pipe(switchMap(function () {
            return loaded$.pipe(take(1), // wait for the next loaded event
            mergeMap(function () { return loadedStateUpdates$; }), // to update the state as usual
        ...
        loaded$.pipe(switchMap(function () { return emitSeekEvents(_this.videoElement, observation$); }), takeUntil(stopContent$)).subscribe(function (evt) {
     ...
     */
    Player.prototype._priv_triggerPositionUpdate = function (observation) {
        var _a;
        if (this._priv_contentInfos === null) {
...
Player.version = /* PLAYER_VERSION */ "3.26.2+fixed_ttml_subs+session_remove_before_close-3";
