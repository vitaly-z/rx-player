...
export default function initMediaKeys(mediaElement, keySystemsConfigs) {
    return getMediaKeysInfos(mediaElement, keySystemsConfigs)
        .pipe(mergeMap(function (mediaKeysInfos) {
        return observableConcat(observableOf({ type: "created-media-keys", value: mediaKeysInfos }), attachMediaKeys(mediaKeysInfos, mediaElement)
            .pipe(mapTo({ type: "attached-media-keys", value: mediaKeysInfos, })));
    }));
}
