...
function buildKeySystemConfigurations(ksName, keySystem) {
    ...
    // persistentState,
    // sessionTypes,
    return [{ initDataTypes: ["cenc"], videoCapabilities: videoCapabilities,
    ...
    return observableDefer(function () {
        ...
        function recursivelyTestKeySystems(index) {
            ...
            if (index >= keySystemsType.length) {
                var error = new EncryptedMediaError("INCOMPATIBLE_KEYSYSTEMS", "No key system compatible with your " +
                    "wanted configuration has been found " +
                    "in the current browser.");
                return observableThrow(error);
            }
            if (requestMediaKeySystemAccess == null) {
                var error = Error("requestMediaKeySystemAccess is not " +
