...
import { map, mapTo, mergeMap, startWith, } from "rxjs/operators";
...
export default function getSession(initializationDataInfo, mediaKeysInfos) {
    return observableDefer(function () {
        ...
        var entry = loadedSessionsStore.get(initData, initDataType);
        if (entry !== null) {
        ...
        return (previousLoadedSession != null ?
            ...
            return observableConcat(cleanOldSessions(loadedSessionsStore), createSession(initData, initDataType, mediaKeysInfos)
 ...
 * @returns {Observable}
 */
function cleanOldSessions(loadedSessionsStore) {
    var maxSessions = EME_MAX_SIMULTANEOUS_MEDIA_KEY_SESSIONS;
    var cleaningOldSessions$ = [];
    if (maxSessions > 0 && maxSessions <= loadedSessionsStore.getLength()) {
        var entries = loadedSessionsStore.getAll();
        for (var i = 0; i < entries.length - maxSessions; i++) {
            var entry = entries[i];
    ...
    return cleaningOldSessions$.length !== 0 ? observableMerge.apply(void 0, cleaningOldSessions$) :
        EMPTY;
}
