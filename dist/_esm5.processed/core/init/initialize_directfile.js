...
import { filter, ignoreElements, map, mergeMap, mergeMapTo, share, take, } from "rxjs/operators";
...
export default function initializeDirectfileContent(_a) {
    ...
    var _b = seekAndLoadOnMediaEvents({ clock$: clock$,
        ...
        mustAutoPlay: autoPlay, setCurrentTime: setCurrentTime, isDirectfile: true }), seek$ = _b.seek$, load$ = _b.load$;
    ...
    var stalled$ = clock$.pipe(map(function (tick) { return tick.stalled === null ? EVENTS.unstalled() :
        EVENTS.stalled(tick.stalled); }));
    // Manage "loaded" event and warn if autoplay is blocked on the current browser
    var loadedEvent$ = emeManager$.pipe(filter(function isEMEReady(evt) {
