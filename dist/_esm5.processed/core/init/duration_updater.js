 ...
 */
import { combineLatest as observableCombineLatest, defer as observableDefer, EMPTY, fromEvent as observableFromEvent, interval as observableInterval, merge as observableMerge, of as observableOf, } from "rxjs";
import { catchError, distinctUntilChanged, filter, ignoreElements, map, mapTo, mergeMap, startWith, switchMap, take, tap, } from "rxjs/operators";
...
export default function DurationUpdater(manifest, mediaSource) {
    return observableDefer(function () {
        ...
        return setMediaSourceDuration(mediaSource, manifest, undefined).pipe(mergeMap(function (initialDurationUpdate) {
...
function whenSourceBuffersEndedUpdates$(sourceBuffers) {
    ...
    for (var i = 0; i < sourceBuffers.length; i++) {
        ...
        sourceBufferUpdatingStatuses.push(observableMerge(observableFromEvent(sourceBuffer, "updatestart").pipe(mapTo(true)), observableFromEvent(sourceBuffer, "update").pipe(mapTo(false)), observableInterval(500).pipe(mapTo(sourceBuffer.updating))).pipe(startWith(sourceBuffer.updating), distinctUntilChanged()));
    }
...
function isMediaSourceOpened$(mediaSource) {
    return observableMerge(onSourceOpen$(mediaSource).pipe(mapTo(true)), onSourceEnded$(mediaSource).pipe(mapTo(false)), onSourceClose$(mediaSource).pipe(mapTo(false))).pipe(startWith(mediaSource.readyState === "open"), distinctUntilChanged());
}
