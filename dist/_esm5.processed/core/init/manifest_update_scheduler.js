 ...
 */
import { defer as observableDefer, EMPTY, from as observableFrom, map, merge as observableMerge, mergeMap, of as observableOf, share, take, timer as observableTimer, } from "rxjs";
    ...
    function handleManifestRefresh$(_a) {
        ...
        var internalRefresh$ = scheduleRefresh$
            ...
            return startManualRefreshTimer(delay !== null && delay !== void 0 ? delay : 0, minimumManifestUpdateInterval, sendingTime)
                .pipe(map(function () { return ({ completeRefresh: completeRefresh, unsafeMode: unsafeMode }); }));
        }));
        /** Emit when the Manifest tells us that it has "expired". */
        var expired$ = manifest.expired === null ?
            EMPTY :
            observableTimer(minInterval).pipe(mergeMap(function () {
                return manifest.expired === null ? EMPTY :
                    observableFrom(manifest.expired);
            }), map(function () { return ({ completeRefresh: true, unsafeMode: unsafeModeEnabled }); }));
        ...
        function createAutoRefreshObservable() {
            ...
            return observableTimer(Math.max(actualRefreshInterval, minInterval))
                .pipe(map(function () { return ({ completeRefresh: false, unsafeMode: unsafeModeEnabled }); }));
        }
    }
    /**
