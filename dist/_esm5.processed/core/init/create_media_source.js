...
var onSourceOpen$ = events.onSourceOpen$;
/**
 * Set the media duration in the mediaSource.
 * @param {MediaSource} mediaSource
 * @param {number} duration
 */
export function setDurationToMediaSource(mediaSource, duration) {
    var newDuration = duration === Infinity ? Number.MAX_VALUE :
        duration;
...
function createMediaSource(mediaElement) {
    return new Observable(function (observer) {
        ...
        var oldSrc = isNonEmptyString(mediaElement.src) ? mediaElement.src :
            null;
 ...
 * @returns {Observable}
 */
export default function openMediaSource(mediaElement) {
    return createMediaSource(mediaElement).pipe(mergeMap(function (mediaSource) {
        return onSourceOpen$(mediaSource).pipe(take(1), mapTo(mediaSource));
    }));
}
