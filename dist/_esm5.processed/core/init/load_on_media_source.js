...
export default function createMediaSourceLoader(_a) {
    ...
    return function loadContentOnMediaSource(mediaSource, initialTime, autoPlay) {
        ...
        var _a = seekAndLoadOnMediaEvents({ clock$: clock$,
            mediaElement: mediaElement, startTime: initialTime,
        ...
        var streamClock$ = createStreamClock(clock$, { autoPlay: autoPlay,
            initialPlay$: initialPlay$, initialSeek$: seek$, manifest: manifest,
            speed$: speed$, startTime: initialTime });
        ...
        var streams$ = StreamOrchestrator({ manifest: manifest, initialPeriod: initialPeriod }, streamClock$, abrManager, sourceBuffersStore, segmentFetcherCreator, bufferOptions).pipe(mergeMap(function (evt) {
            switch (evt.type) {
                case "end-of-stream":
        ...
        var internalSeek$ = clock$.pipe(tap(function (clockTick) {
            if (clockTick.stalled !== null &&
                ...
                var discontinuity = getDiscontinuity(clockTick, manifest);
        ...
        return observableMerge(internalSeek$, loadedEvent$, playbackRate$, stalled$, streams$, streamEvents$).pipe(finalize(function () {
