...
import { filter, finalize, ignoreElements, mergeMap, switchMap, takeUntil, } from "rxjs/operators";
...
export default function createMediaSourceLoader(_a) {
    ...
    return function loadContentOnMediaSource(mediaSource, initialTime, autoPlay) {
        ...
        var _b = initialSeekAndPlay({ clock$: clock$, mediaElement: mediaElement, startTime: initialTime,
            mustAutoPlay: autoPlay, setCurrentTime: setCurrentTime, isDirectfile: false }), seekAndPlay$ = _b.seekAndPlay$, initialPlayPerformed = _b.initialPlayPerformed, initialSeekPerformed = _b.initialSeekPerformed;
        var streamEvents$ = initialPlayPerformed.asObservable().pipe(filter(function (hasPlayed) { return hasPlayed; }), mergeMap(function () { return streamEventsEmitter(manifest, mediaElement, clock$); }));
        var streamClock$ = createStreamClock(clock$, { autoPlay: autoPlay, initialPlayPerformed: initialPlayPerformed, initialSeekPerformed: initialSeekPerformed, manifest: manifest, speed$: speed$, startTime: initialTime });
         ...
         */
        var stallAvoider$ = StallAvoider(clock$, mediaElement, manifest, discontinuityUpdate$, lockedStream$, setCurrentTime);
        /**
