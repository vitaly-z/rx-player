            ...
            switch (arg.type) {
                case "metrics": {
                    ...
                    if (size != null && duration != null) {
                        requests$.next({ type: "metrics", value: { size: size, duration: duration, content: content } });
                    }
                    break;
                }
                case "request": {
                    ...
                    requests$.next({ type: "requestBegin", value: { duration: duration,
                            time: time, requestTimestamp: performance.now(), id: id } });
                    break;
                }
                case "progress": {
                    var value = arg.value;
                    if (value.totalSize != null && value.size < value.totalSize) {
                        requests$.next({ type: "progress", value: { duration: value.duration,
                                size: value.size,
