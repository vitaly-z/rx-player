...
import { concat as observableConcat, defer as observableDefer, interval as observableInterval, mapTo, merge as observableMerge, of as observableOf, startWith, Subject, switchMapTo, takeUntil, } from "rxjs";
...
function generateRefreshInterval(videoElement) {
    ...
    var autoRefresh$ = observableInterval(MAXIMUM_HTML_TEXT_TRACK_UPDATE_INTERVAL)
        .pipe(startWith(null));
    return manualRefresh$.pipe(startWith(null), switchMapTo(observableConcat(autoRefresh$.pipe(mapTo(true), takeUntil(seeking$)), observableOf(false))));
}
/**
