...
var VideoThumbnailLoader = /** @class */ (function () {
    ...
    VideoThumbnailLoader.prototype._startTimeSettingTask = function (contentInfos, segments, time) {
        ...
        var segmentLoader = createSegmentLoader(loader.video.loader, undefined, { baseDelay: 0,
            maxDelay: 0,
            maxRetryOffline: 0,
            maxRetryRegular: 0 });
        var segmentParser = loader.video.parser;
        var taskPromise = lastValueFrom(observableRace(abortError$, getInitializedSourceBuffer$(contentInfos, this._videoElement, { segmentLoader: segmentLoader,
            segmentParser: segmentParser }).pipe(mergeMap(function (videoSourceBuffer) {
            ...
            var segmentsLoading$ = loadSegments(segments, segmentLoader, contentInfos);
            return observableMerge(bufferCleaning$.pipe(ignoreElements()), segmentsLoading$).pipe(mergeMap(function (arr) {
                return combineLatest(arr.map(function (_a) {
                    ...
                    return pushData(inventoryInfos, segmentParser, data, videoSourceBuffer)
                        .pipe(tap(function () {
