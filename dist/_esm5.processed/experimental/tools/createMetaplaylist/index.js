 ...
 */
import { combineLatest as observableCombineLatest, lastValueFrom, map, of as observableOf, } from "rxjs";
...
function createMetaplaylist(contentsInfos, timeOffset) {
    ...
    var completeContentsInfos$ = contentsInfos.map(function (contentInfos) {
        ...
        if (duration !== undefined) {
            return observableOf({ url: url, transport: transport, duration: duration });
        }
    ...
    return lastValueFrom(observableCombineLatest(completeContentsInfos$).pipe(map(function (completeContentsInfos) {
