 ...
 * RxJS Observables
 */
import { NEVER, combineLatest as observableCombineLatest, defer as observableDefer, delay, distinctUntilChanged, fromEvent as observableFromEvent, interval as observableInterval, map, merge as observableMerge, of as observableOf, startWith, switchMap, throttleTime, } from "rxjs";
...
function onPictureInPictureEvent$(elt) {
    return observableDefer(function () {
        ...
        if (mediaElement.webkitSupportsPresentationMode === true &&
            ...
            return observableFromEvent(mediaElement, "webkitpresentationmodechanged").pipe(map(function () { return ({
        ...
        return observableMerge(observableFromEvent(mediaElement, "enterpictureinpicture")
            ...
            .pipe(map(function () { return ({ isEnabled: false, pipWindow: null }); }))).pipe(startWith(initialState));
    });
}
/**
...
export { isPageActive, isVideoVisible, videoWidth$, onPlayPause$, onTextTrackChanges$, onLoadedMetadata$, onSeeking$, onSeeked$, onEnded$, onTimeUpdate$, onFullscreenChange$, onSourceOpen$, onSourceClose$, onSourceEnded$, onUpdate$, onRemoveSourceBuffers$, onEncrypted$, onKeyMessage$, onKeyAdded$, onKeyError$, onKeyStatusesChange$, onPictureInPictureEvent$, };
