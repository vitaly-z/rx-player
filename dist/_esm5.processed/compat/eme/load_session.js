 ...
 */
import { defer as observableDefer, map, mergeMap, of as observableOf, race as observableRace, take, timer as observableTimer, } from "rxjs";
import log from "../../log";
...
export default function loadSession(session, sessionId) {
    return observableDefer(function () {
        ...
        return tryCatch(function () { return castToObservable(session.load(sessionId)); }, undefined);
    }).pipe(mergeMap(function (isLoaded) {
        ...
        return observableRace(observableTimer(EME_WAITING_DELAY_LOADED_SESSION_EMPTY_KEYSTATUSES), onKeyStatusesChange$(session)).pipe(take(1), map(function () { return isLoaded; }));
    }));
}
